...
def translateDescription():
    conn = pymysql.connect(host='localhost', user='root', password='', db='omim', charset="utf8")
    cur = conn.cursor()

    sqla = '''SELECT pid,description FROM p_t_copy1 JOIN type_copy1 USING (tid) WHERE stop=0 and pid>=38033;'''
    try:
        cur.execute(sqla)
        result=cur.fetchall()
    except:
    ...
    for i in range(len(result)):
        ...
        for item in range(1, len(sys.argv)):
            description += sys.argv[item] + " "
        js = Py4Js()
        res = translate(js, description)
        # print(res)
        sqlb = '''UPDATE p_t_copy1 SET des_ch="''' + res + '''" WHERE pid=''' + str(pid) + ''';'''
        try:
            cur.execute(sqlb)
            conn.commit()
            print(pid,"插入翻译结果成功")
